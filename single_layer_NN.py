...
def GetSimple1LayerNNWithTrainingHistory(df=None,epochs=3):
    ...
    df = df[['YEAR OCC', 'MONTH OCC', 'HOUR OCC', 'AREA', 'Rpt Dist No', 'Crm Cd', 'Vict Age', 'LAT', 'LON']]
    
    ## Added so we can compare results
    ...
    nn = SimpleSingleLayerNN(input_shape= len(X.columns)  
                              ,output_len=len(y.columns)  
                              )
    training_history = nn.model.fit(X_train
                                    , y_train
                                    , epochs= epochs
                                    , batch_size=32
                                    , shuffle= True
    ...
    # ##%% 
    # # input_columns = ['YEAR OCC','MONTH OCC','DAY OCC', 'HOUR OCC','AREA','Crm Cd'] ##OLD
    # input_columns = ['YEAR OCC', 'MONTH OCC', 'HOUR OCC', 'AREA', 'Rpt Dist No', 'Crm Cd', 'Vict Age']
...
if __name__ == '__main__':
    ...
    singleLayerNN,singleLayerNN_training_history = GetSimple1LayerNNWithTrainingHistory(epochs=3)
